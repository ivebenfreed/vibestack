...
export async function copyToClipboard(
  ...
  try {
    ...
    // Check for any type of selection: cells, ranges, rows, or external row selection
    const hasSelection = selectedCells.size > 0 || 
                        activeRange || 
                        selectedRows.size > 0 || 
                        (externalSelectedIds && externalSelectedIds.size > 0)
    
    if (onlySelected && !hasSelection) {
      console.warn('[Clipboard] No cells or rows selected for copy operation')
      return false
    }
    ...
    if (activeRange) {
    ...
    } else if (externalSelectedIds && externalSelectedIds.size > 0) {
      // Handle row-level selection from main table
      const selectedRowIndices = Array.from(externalSelectedIds)
        .map(id => {
      ...
      if (selectedRowIndices.length > 0) {
        ...
        console.warn('[Clipboard] No valid rows found for selected IDs')
        return false
      }
    } else {
      // Select all data
    ...
    // Add data rows
    if (externalSelectedIds && externalSelectedIds.size > 0 && !activeRange && selectedCells.size === 0) {
    ...
    for (const item of clipboardItems) {
      ...
      if (item.types.includes('application/json')) {
        ...
        try {
          ...
          console.warn('[Clipboard] Invalid JSON format, trying other formats')
        }
      }
      ...
      if (item.types.includes('text/html')) {
        ...
        try {
          ...
          console.warn('[Clipboard] Invalid HTML table format, trying plain text')
        }
      }
