/**
 * Worker-compatible metadata filter for Cloudflare Workers
 * No file system access - relies on imported entities only
 */

import { getMetadataArgsStorage } from 'typeorm';
import { isServerOnly, isClientOnly, isServerEntity, isClientEntity, METADATA_KEYS } from './context.js';

export class WorkerMetadataFilter {
    private metadataStorage = getMetadataArgsStorage();

    /**
     * Get all entities that have been imported/registered
     * In worker environment, we rely on entities being imported beforehand
     */
    getAllEntities() {
        return this.metadataStorage.tables.filter(table => {
            // Filter out junction tables and other non-entity tables
