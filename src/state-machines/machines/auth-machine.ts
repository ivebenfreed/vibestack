...
export const authMachine = setup({
  ...
  states: {
    ...
    determiningInitialState: {
      ...
      always: [
        {
          // CRITICAL FIX: Don't trust persisted session expiry alone - always validate with server first
          // This prevents the loop where expired sessions keep getting restored
          target: 'checking',
          guard: ({ context }) => {
            ...
            // Only proceed if session appears valid, but always check with server
            fileLog.info(`[AuthMachine] Session appears valid, checking with server: ${context.user?.email}`);
            return sessionValid;
          },
          actions: [
            ({ context }) => fileLog.info(`[AuthMachine] 🔍 TRANSITION: Validating persisted session with server: ${context.user?.email}`),
          ]
        },
        {
          // REMOVED: This guard could cause the same loop issue - always validate session with server first
          // Let the checking state handle all session validation logic consistently
        },
        {
          // REMOVED: This guard could cause the same loop issue - always validate session with server first
        },
      ...
      on: {
        ...
        SESSION_FAULT: {
          ...
          actions: [
            ...
            // CRITICAL: Clear persisted state when session fault is detected
            () => {
              try {
                localStorage.removeItem('auth-machine-snapshot');
                localStorage.removeItem('auth-machine-state');
                fileLog.info('[AuthMachine] Cleared persisted state due to session fault');
              } catch (error) {
                fileLog.error('[AuthMachine] Failed to clear persisted state on session fault:', error);
              }
            }
          ]
        },
