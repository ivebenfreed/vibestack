...
const fileLog = log('components/custom/legendtable/renderers/core/TableRenderer.ts');
...
export class TableRenderer {
  ...
  setColumns(columns: Column[]): void {
    fileLog.warn('TableRenderer: setColumns is deprecated - column data comes from state machine coordinate mapping');
  }

  // Set or update column visibility - DEPRECATED: Now handled by state machine coordinate mapping
  setColumnVisibility(visibility: Record<string, boolean>): void {
    fileLog.warn('TableRenderer: setColumnVisibility is deprecated - column visibility comes from state machine coordinate mapping');
  }
  ...
  setColumnOrder(order: string[]): void {
    fileLog.warn('TableRenderer: setColumnOrder is deprecated - column order comes from state machine coordinate mapping');
  }
  ...
  updateRow(rowId: string, newData: any, relationshipResolvers?: Record<string, (id: string | string[]) => string>): void {
    const lastRenderState = this.stateManager.getLastRenderState();
    if (!lastRenderState) {
      fileLog.warn('TableRenderer: updateRow - no render state available');
      return;
    }
    
    fileLog.info('🔧 TableRenderer: Updating single row', {
      rowId,
  ...
  updateCell(rowId: string, columnId: string, newValue: any, relationshipResolvers?: Record<string, (id: string | string[]) => string>): void {
    const lastRenderState = this.stateManager.getLastRenderState();
    if (!lastRenderState) {
      fileLog.warn('TableRenderer: updateCell - no render state available');
      return;
    }
    ...
    if (!column) {
      fileLog.warn('TableRenderer: updateCell - column not found:', columnId);
      return;
    }
    
    fileLog.info('🔧 TableRenderer: Updating single cell', {
      rowId,
      columnId,
      newValue,
  ...
  updateColumnWidth(columnId: string, width: number): void {
    fileLog.warn('TableRenderer: updateColumnWidth is deprecated - column updates should come from state machine');
  ...
  updateCoordinateMapping(coordinateMapping: any, version: number): void {
    fileLog.info('TableRenderer: Updating coordinate mapping from state machine:', {
      version,
