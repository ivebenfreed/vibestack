...
export function UltraTable({
  ...
  const selection = useUltraTableSelection({
    ...
    selectedIds // Pass existing table selection to keyboard handler
  })

  // Subscribe to Legend State observables for CSS application
  const selectedCells = use$(selectionState$.selectedCells)
  const focusedCell = use$(selectionState$.focusedCell)
  
  // Apply selection highlighting directly to cells (avoiding separate component)
  React.useEffect(() => {
    console.log('[UltraTable] CSS application useEffect triggered', {
      hasTable: !!tableElementRef.current,
      selectedCells: Array.from(selectedCells),
      focusedCell: focusedCell
    })
    
    if (!tableElementRef.current) {
      console.log('[UltraTable] No table element ref, skipping CSS application')
      return
    }
    
    // Clear all previous highlights
    const allCells = tableElementRef.current.querySelectorAll('td')
    console.log('[UltraTable] Found total cells:', allCells.length)
    ...
    console.log('[UltraTable] Applying selection to cells:', Array.from(selectedCells))
    selectedCells.forEach(cellKey => {
      const [row, col] = cellKey.split(':').map(Number)
      console.log('[UltraTable] Looking for cell:', { row, col, cellKey })
      
      // Try multiple selector strategies
      const cellElement = tableElementRef.current?.querySelector(`td[data-row="${row}"][data-col="${col}"]`) ||
                         tableElementRef.current?.querySelector(`td:nth-child(${col + 1})`)
      
      console.log('[UltraTable] Found cell element:', !!cellElement, cellElement?.getAttribute('data-row'), cellElement?.getAttribute('data-col'))
      
      if (cellElement) {
        cellElement.setAttribute('data-selected', 'true')
        cellElement.classList.add('ultra-table-selected')
        console.log('[UltraTable] Applied selection CSS to cell:', cellKey)
      } else {
        console.warn('[UltraTable] Could not find cell element for:', cellKey)
      }
    })
    
    // Apply focus highlight
    if (focusedCell) {
      const cellElement = tableElementRef.current?.querySelector(
        `td[data-row="${focusedCell.row}"][data-col="${focusedCell.col}"]`
      )
      console.log('[UltraTable] Applying focus to cell:', focusedCell, !!cellElement)
      if (cellElement) {
  ...
  const handleCellClick = useCallback((rowIndex: number, columnIndex: number, field: string, event?: MouseEvent) => {
    if (options.enableSelection !== false) {
      ...
      // Remove automatic row selection - let users explicitly select rows if needed
  ...
  return (
    <div ref={selection.tableRef} className={cn('border rounded-lg bg-background overflow-hidden', className)}>
      {/* Selection CSS Styles */}
      <style>{`
        .ultra-table-selected {
