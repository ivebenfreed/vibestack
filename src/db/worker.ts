...
const fileLog = log('db/worker.ts');
...
worker({
  async init() {
    fileLog.info('🔄 Initializing PGlite worker with IndexedDB storage...');
    ...
    while (retries < maxRetries) {
      try {
        ...
        fileLog.info('🔄 Creating/opening database...');
        const db = await PGlite.create(config);
        
        // Create the uuid-ossp extension if needed
        fileLog.info('🔄 Creating uuid-ossp extension...')
        await db.exec('CREATE EXTENSION IF NOT EXISTS "uuid-ossp";');
        
        // Initialize live extension
        fileLog.info('🔄 Initializing live extension...');
        if (db.live) {
          fileLog.info('✅ Live extension initialized successfully');
        } else {
          fileLog.warn('⚠️ Live extension not available');
        }
        
        fileLog.info('✅ Database initialized successfully');
        return db;
        
      } catch (err: any) {
        retries++;
        fileLog.error(`❌ Database initialization attempt ${retries} failed:`, err);
        
        // Handle specific PGlite/IndexedDB errors
        if (err.errno === 44 || err.name === 'ErrnoError') {
          fileLog.warn(`⚠️ IndexedDB busy error (errno ${err.errno}), retrying in ${retries * 1000}ms...`);
        ...
        if (retries >= maxRetries) {
          fileLog.error('❌ Failed to initialize database after max retries');
          throw err;
        }
      }
    }
