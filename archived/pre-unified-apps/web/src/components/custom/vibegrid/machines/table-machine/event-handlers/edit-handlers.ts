...
const log = uiLog('components/custom/vibegrid/machines/table-machine/event-handlers/edit-handlers.ts');
...
function ensureEditingOverlay(context: any, self?: any): EditingOverlay {
  ...
  if (needsRecreation) {
    ...
    if (globalEditingOverlay) {
      log.info('EditHandlers: Container changed, cleaning up old EditingOverlay');
    ...
    const config = {
      onCommit: (value: any) => {
        ...
        if (self && self.getSnapshot && self.getSnapshot().status !== 'stopped') {
          try {
            ...
            log.warn('EditHandlers: Could not send EDIT_COMMIT, machine may be stopped:', error);
          }
        }
      },
      onCancel: () => {
        ...
        if (self && self.getSnapshot && self.getSnapshot().status !== 'stopped') {
          try {
            ...
            log.warn('EditHandlers: Could not send EDIT_CANCEL, machine may be stopped:', error);
          }
        }
      },
      zIndex: 1000,
      relationshipContext: {
        ...
        getStore: () => {
          ...
          log.info('EditHandlers: getStore called', { 
    ...
    log.info('EditHandlers: Created new EditingOverlay instance', {
      container,
...
function cleanupEditingOverlay(): void {
  if (globalEditingOverlay) {
    globalEditingOverlay.destroy();
    globalEditingOverlay = null;
    log.info('EditHandlers: Cleaned up EditingOverlay instance');
  }
}
...
export const editHandlers = {
  ...
  'edit.cell.start.single': {
    actions: [
      ...
      ({ context, event }) => {
        log.info('🎯 Edit Handler: Single-click edit initiated', {
          rowId: event.rowId,
          columnId: event.columnId,
      ...
      ({ context, event, self }) => {
        ...
        if (!column) {
          log.warn('🎯 Edit Handler: Column not found', { columnId: event.columnId });
          return;
        }
        
        if (column.editable === false) {
          log.warn('🎯 Edit Handler: Column not editable', { columnId: event.columnId });
          return;
        }
        ...
        log.info('🎯 Edit Handler: Starting single-click edit', {
          cell,
          value,
          column: column.name
        });
  ...
  'edit.start': {
    actions: [
      ...
      ({ context, event, self }) => {
        try {
          log.info('EditHandlers: About to ensure EditingOverlay', {
          ...
          if (!column) {
            log.error('EditHandlers: Column not found for cell', event.cell);
            return;
          }
          ...
          log.info('EditHandlers: Showing editor directly', {
            cell: event.cell,
            column: column.id,
            value: event.value,
            position,
          ...
          log.error('EditHandlers: Error showing editor', error);
        }
      },
      ...
      ({ event }) => {
        log.info('TableMachine: Edit started', {
          cell: event.cell,
          value: event.value
        });
      }
  ...
  'edit.update': {
    actions: [
      ...
      ({ context, event }) => {
        log.info('TableMachine: Edit value updated', {
  ...
  'edit.cancel': {
    actions: [
      ...
      ({ context }) => {
        try {
          if (globalEditingOverlay) {
            ...
            log.info('EditHandlers: Editor hidden directly');
          }
        } catch (error) {
          log.error('EditHandlers: Error hiding editor', error);
        }
      },
      ...
      ({ context }) => {
        log.info('TableMachine: Edit cancelled', {
          cell: context.editingCell
        });
      }
    ]
  ...
  'edit.validate': {
    actions: [
      ...
      ({ context, event }) => {
        log.info('TableMachine: Edit validation requested', {
          cell: context.editingCell,
          value: event.value
        });
      }
  ...
  'edit.error': {
    actions: [
      ...
      ({ context, event }) => {
        try {
          if (globalEditingOverlay) {
            ...
            log.info('EditHandlers: Validation errors updated directly');
          }
        } catch (error) {
          log.error('EditHandlers: Error updating validation errors', error);
        }
      },
      ...
      ({ event }) => {
        log.error('TableMachine: Edit error', {
          errors: event.errors,
  ...
  'edit.operation.remove': {
    actions: [
      ...
      ({ event }) => {
        log.info('TableMachine: Optimistic operation removed', {
  ...
  'edit.operation.update': {
    actions: [
      ...
      ({ event }) => {
        log.info('TableMachine: Optimistic operation updated', {
          operationId: event.operationId,
          status: event.status
        });
      }
  ...
  'EDIT_UPDATE': {
    actions: [
      ({ event }) => {
        log.info('TableMachine: Edit update from canvas', {
          value: event.value
        });
      },
  ...
  'EDIT_COMMIT': {
    guard: ({ context, event }) => {
      ...
      log.info('TableMachine: Edit commit guard check', {
    ...
    actions: [
      ({ event }) => {
        log.info('TableMachine: Edit commit from canvas', {
          value: event.value
        });
      },
      ...
      ({ context, event, self }) => {
        ...
        if (entityIndex !== -1 && rowIndex !== -1) {
          ...
          if (resolvedFields.length > 0) {
            log.info('TableMachine: Current resolved fields before update', {
              rowId,
              resolvedFields,
          ...
          log.info('TableMachine: Checking if column needs relationship resolution', {
            columnId,
          ...
          if (column && (column.cellType || column.type)?.startsWith('relationship')) {
            ...
            log.info('TableMachine: Attempting to resolve relationship', {
              columnId,
            ...
            if (resolver && event.value != null) {
              try {
                ...
                log.info('TableMachine: Successfully resolved relationship value', {
                  columnId,
                ...
                log.error('TableMachine: Error resolving relationship', {
                  columnId,
                  value: event.value,
                  error
                });
              ...
              log.warn('TableMachine: Cannot resolve relationship - missing resolver or null value', {
                columnId,
                hasResolver: !!resolver,
                value: event.value
              });
          ...
          if (!hasEntityUpdateHandler) {
            ...
            log.info('TableMachine: Updated row data optimistically (no entity handler)', {
              rowId,
              field,
              newValue: event.value,
              oldValue: context.originalValue
            ...
            if (self && self.send) {
              log.info('TableMachine: Triggering view processing after edit');
              self.send({ type: 'INVOKE_VIEW_ACTOR' });
            }
          } else {
            log.info('TableMachine: Skipping optimistic update - entity handler will trigger updates', {
              rowId,
              field,
              newValue: event.value
            });
          ...
          if (context.onEntityUpdate && typeof context.onEntityUpdate === 'function') {
            ...
            log.info('TableMachine: Using onEntityUpdate for update', {
              rowId,
              updates,
              field
            });
            ...
            if (updateResult instanceof Promise) {
              updateResult
                .then(() => {
                  log.info('TableMachine: Entity update successful', { rowId, field });
                ...
                .catch((error: any) => {
                  log.error('TableMachine: Entity update failed', {
                    rowId,
                    field,
                    error
                  });
              ...
              log.info('TableMachine: Entity update successful (sync)', { rowId, field });
              self.send({
                type: 'edit.operation.remove',
                operationId
              });
            ...
            log.warn('TableMachine: No onEntityUpdate handler provided', {
              rowId,
              field,
              value: event.value
            });
          ...
          log.warn('TableMachine: Could not find entity or row to update', {
            rowId,
            entityIndex,
            rowIndex
          });
    ...
    actions: [
      ...
      ({ context }) => {
        try {
          if (globalEditingOverlay) {
            globalEditingOverlay.hide();
            log.info('EditHandlers: Editor hidden after delay');
          }
        } catch (error) {
          log.error('EditHandlers: Error hiding editor', error);
        }
  ...
  'EDIT_CANCEL': {
    actions: [
      ({ event }) => {
        log.info('TableMachine: Edit cancel from canvas');
      },
  ...
  'edit.ensure.end': {
    ...
    actions: [
      ({ context }) => {
        log.info('TableMachine: Ensuring edit mode ends', {
