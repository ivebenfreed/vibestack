...
export interface OrgEntityDefinition {
  name: string;
  extends: 'base_projects' | 'base_tasks' | 'base_events' | 'base_contacts' | 'base_records' | 'base_documents' | 'base_files' | 'base_activities' | 'base_discussions' | 'base_collections';
...
export class OrgSchemaManager {
  ...
  private validateEntityDefinition(entityName: string, definition: OrgEntityDefinition): string[] {
    ...
    if (!definition.extends || !['base_projects', 'base_tasks', 'base_events', 'base_contacts'].includes(definition.extends)) {
      errors.push(`${entityName}: extends must be one of base_projects, base_tasks, base_events, base_contacts`);
    }
