...
docker exec elevra-main-postgres pg_dump -U postgres elevra_dev > "$BACKUP_FILE"
...
if [ -f "$BACKUP_FILE" ] && [ -s "$BACKUP_FILE" ]; then
    ...
    echo "   cat $BACKUP_FILE | docker exec -i elevra-main-postgres psql -U postgres -d elevra_dev"
else
    echo "❌ Backup failed"
    rm -f "$BACKUP_FILE"
    exit 1
