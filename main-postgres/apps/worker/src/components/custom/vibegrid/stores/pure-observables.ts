...
const vibeLog = log('components/custom/vibegrid/stores/pure-observables.ts');
...
function applyGrouping(rows: any[], groupConfig: GroupConfig): any[] {
  ...
  vibeLog.debug('🎯 Applying grouping', { 
  ...
  vibeLog.debug('🎯 Grouping applied', {
...
function generateTableStateKey(entityType: string): string | null {
  try {
    ...
    if (!orgId || !userId || orgId === 'universe') {
      vibeLog.debug('🔧 Cannot generate storage key - missing context', { orgId, userId });
      return null;
    }
    ...
    vibeLog.debug('🔧 Generated table state storage key', { storageKey, entityType, orgId, userId });
    
    return storageKey;
  } catch (error) {
    vibeLog.error('❌ Error generating storage key', { error: error.message, entityType });
    return null;
  }
}
...
function transformTableState(loaded: any, defaultState: Partial<PersistedTableState>): PersistedTableState {
  if (!loaded || typeof loaded !== 'object') {
    vibeLog.debug('🔧 No saved state found, using defaults');
    return defaultState as PersistedTableState;
  }
  
  // Handle version migrations
  if (loaded.version !== '1.0') {
    vibeLog.debug('🔧 Migrating table state from version', loaded.version, 'to 1.0');
  ...
  vibeLog.debug('🔧 Restored table state with', {
...
function loadDisplayState(entityType: string): Partial<PersistedTableState> | null {
  try {
    ...
    vibeLog.debug('🔧 Loading display state', { entityType, entityName, storageKey });
    
    const stored = localStorage.getItem(storageKey);
    if (!stored) {
      vibeLog.debug('🔧 No stored display state found', { storageKey });
      return null;
    }
    
    const parsed = JSON.parse(stored);
    vibeLog.debug('🔧 Loaded display state from localStorage', { storageKey, state: parsed });
    return parsed;
    
  } catch (error) {
    vibeLog.error('❌ Error loading display state', { entityType, error: error.message });
    return null;
  }
}
...
function saveDisplayState(entityType: string, tableState: any): void {
  try {
    ...
    vibeLog.debug('🎯 TableCore: Saved persistent display state', { storageKey, displayState });
    
  } catch (error) {
    vibeLog.error('❌ Error saving display state', { entityType, error: error.message });
  }
}
...
export function createTableCore$(entityType: string, columns: Column[]) {
  vibeLog.info('🎯 Creating tableCore$ observable', { entityType, columnCount: columns.length });
  
  // Create default state structure
  const defaultState = createDefaultTableState(entityType, columns);
  vibeLog.debug('🎯 Default state created', { entityType, defaultSortBy: defaultState.sortBy });
  ...
  const tableCore$ = observable({
    ...
    get processedRows() {
      ...
      if (loading || !schema?.entities || !schema.entities[entityType]) {
        vibeLog.debug('⏳ Schema not ready for processedRows', { entityType, loading });
        return [];
      }
      ...
      if (entityObs) {
        try {
          ...
          data = entityObs.get() || {};
          vibeLog.debug('✅ Got entity data', {
            entityType,
            recordCount: Object.keys(data || {}).length
          });
        } catch (error) {
          vibeLog.error('❌ Failed to get entity data', { entityType, error: error.message });
          data = {};
        }
      } else {
      ...
      vibeLog.debug('🎯 Processed rows computed', {
    ...
    toggleSort(field: string, isMultiSort: boolean = false) {
      ...
      vibeLog.debug('toggleSort called', { 
        field, 
        isMultiSort, 
        currentSort: current, 
        fieldIndex: index,
      ...
      batch(() => {
        if (index === -1) {
          ...
          if (isMultiSort) {
            ...
            vibeLog.debug('Adding new sort field to multi-sort', { newSort });
            tableCore$.sortBy.set(newSort);
          } else {
            const newSort = [{ field, direction: 'asc' }];
            vibeLog.debug('Setting single sort field', { newSort });
            tableCore$.sortBy.set(newSort);
          }
          ...
          vibeLog.debug('Changing sort direction to desc', { field, newSort });
          tableCore$.sortBy.set(newSort);
        } else {
          // Third click: remove this sort field entirely (allow unsorted state)
          const newSort = current.filter((_, i) => i !== index);
          vibeLog.debug('Removing sort field', { field, newSort });
          tableCore$.sortBy.set(newSort);
        }
      ...
      try {
        ...
        vibeLog.debug('localStorage content after sort change', {
          storageKey: `vibestack-table-${entityType}`,
          storedValue: stored,
          parsedValue: stored ? JSON.parse(stored) : null
        });
      } catch (e) {
        vibeLog.error('Failed to read localStorage after sort change', { error: e.message });
      }
    },
    ...
    setFilter(field: string, value: any, operator: string = 'contains') {
      ...
      vibeLog.debug('🎯 Filter set', { field, value, operator });
    },
    ...
    clearFilter(field: string) {
      ...
      vibeLog.debug('🎯 Filter cleared', { field });
    },
    ...
    toggleColumn(columnId: string) {
      ...
      vibeLog.debug('🎯 Column toggled', { columnId, visible: !visibility[columnId] });
    },
    ...
    setColumnWidth(columnId: string, width: number) {
      ...
      vibeLog.debug('🎯 Column width set', { columnId, width });
    },
    ...
    updateColumnWidth(columnId: string, width: number) {
      ...
      vibeLog.debug('🎯 Column width updated', { columnId, width });
    },
    
    setGroupConfig(config: GroupConfig | null) {
      tableCore$.groupConfig.set(config);
      // persistObservable automatically persists changes
      vibeLog.debug('🎯 Group config set', { config });
    },
    ...
    toggleGroupExpansion(groupId: string) {
      ...
      if (expandedGroups.has(groupId)) {
        ...
        vibeLog.debug('🎯 Group collapsed', { groupId });
      } else {
        expandedGroups.add(groupId);
        vibeLog.debug('🎯 Group expanded', { groupId });
      }
    ...
    expandAllGroups() {
      ...
      vibeLog.debug('🎯 Expand all groups requested');
    ...
    collapseAllGroups() {
      ...
      vibeLog.debug('🎯 All groups collapsed');
    },
    ...
    showAllColumns() {
      ...
      // persistObservable automatically persists changes
      vibeLog.debug('🎯 All columns shown');
    },
    ...
    hideAllColumns() {
      ...
      // persistObservable automatically persists changes
      vibeLog.debug('🎯 All hideable columns hidden');
    },
  ...
  try {
    vibeLog.debug('Configuring localStorage persistence', { entityType, storageKey });
    ...
    vibeLog.debug('localStorage persistence configured', { 
      entityType,
      storageKey,
    ...
    // console.error removed - using vibeLog.error instead
    vibeLog.error('❌ Failed to configure persistence', { entityType, error: error.message });
  }
...
export function createTableInteraction$(tableCore$?: any) {
  vibeLog.debug('🎯 Creating tableInteraction$ observable');
  ...
  const tableInteraction$ = observable({
    ...
    selectCell(cellId: string, isMulti: boolean = false) {
      ...
      // Cell selection logging removed for performance
    },
    ...
    toggleCellSelection(rowId: string, columnId: string, isCtrlKey: boolean = false, isShiftKey: boolean = false) {
      ...
      // Cell selection toggle logging removed for performance
    },
    ...
    selectRow(rowId: string, isMulti: boolean = false) {
      ...
      // Row selection logging removed for performance
    },
    ...
    selectRange(startCell: string, endCell: string) {
      batch(() => {
        ...
        if (tableCore$) {
          ...
          // Range selection logging removed for performance
        } else {
    ...
    startDragSelection(cellId: string) {
      ...
      // Drag selection logging removed for performance
    },
    ...
    updateDragSelection(cellId: string) {
      ...
      batch(() => {
        ...
        if (tableCore$) {
          ...
          // Drag range calculation logging removed for performance
        } else {
          // Fallback to just start and end cells if no tableCore$ access
          cells.add(startCell);
          cells.add(cellId);
      ...
      // Drag selection update logging removed for performance
    },
    ...
    clearSelection() {
      ...
      // Selection clear logging removed for performance
    },
    ...
    startEdit(cellId: string, initialValue?: any) {
      ...
      // Edit start logging removed for performance
    },
    ...
    async saveEdit(valueOverride?: any) {
      ...
      if (cellId) {
        ...
        vibeLog.debug('🎯 Saving edit', { cellId, value, rowId, columnId });
        ...
        try {
          ...
          vibeLog.debug('🔄 Attempting to save edit', { entityType, rowId, columnId, value });
          ...
          vibeLog.debug('🔄 Using entityOperations.updateEntity', { 
          ...
          vibeLog.info('✅ Edit saved successfully', { entityType, rowId, columnId, value });
          
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
          vibeLog.error('❌ Error saving edit', { cellId, value, error: errorMessage });
    ...
    cancelEdit() {
      ...
      // Edit cancel logging removed for performance
    },
    ...
    startDrag(sourceId: string) {
      ...
      // Drag start logging removed for performance
    },
    ...
    endDrag() {
      ...
      if (source && target && source !== target) {
        // Drag complete logging removed for performance
    ...
    startColumnResize(columnId: string, startX: number, startWidth: number) {
      ...
      // Column resize logging removed for performance
    },
    ...
    openHeaderMenu(columnId: string, position: { x: number; y: number }, menuType: 'filter' | 'sort' | 'settings') {
      ...
      // Header menu logging removed for performance
    },
    ...
    openContextMenu(context: 'cell' | 'row' | 'column' | 'header', targetId: string, position: { x: number; y: number }) {
      ...
      // Context menu logging removed for performance
    },
    ...
    openColumnVisibilityMenu() {
      ...
      // Column visibility menu logging removed for performance
    },
    ...
    openGroupConfigMenu() {
      ...
      // Group config menu logging removed for performance
    },
...
export function createTableViewport$(tableCore$?: any) {
  vibeLog.debug('🎯 Creating tableViewport$ observable');
  ...
  const tableViewport$ = observable({
    ...
    updateScroll(top: number, left: number) {
      ...
      // Scroll update logging removed for performance
    },
    ...
    updateViewport(width: number, height: number) {
      ...
      vibeLog.debug('🎯 Viewport updated', { width, height });
    },
    ...
    updateContent(width: number, height: number) {
      ...
      vibeLog.debug('🎯 Content dimensions updated', { width, height });
    },
    ...
    scrollToRow(rowIndex: number) {
      ...
      // Scroll-to-row logging removed for performance
    },
    ...
    scrollToColumn(columnIndex: number, columnWidths: number[]) {
      ...
      // Scroll-to-column logging removed for performance
    },
    ...
    ensureCellVisible(rowIndex: number, columnIndex: number, columnWidths: number[]) {
      ...
      // Cell visibility logging removed for performance
    }
  });
...
export function createPureObservables(entityType: string, columns: Column[]) {
  vibeLog.info('🎯 Creating VibeGrid observables', { entityType, columns: columns.length });
