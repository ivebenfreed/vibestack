...
export class DataForgeEntityManager {
   ...
   */
  private async resolveReferences(orgId: string, entityName: string, results: any[]): Promise<any[]> {
    ...
    try {
      ...
      console.warn(`[EntityManager] Failed to resolve references for ${entityName}:`, error);
   ...
   */
  private async getEntityMetadata(orgId: string, entityName: string): Promise<any> {
    try {
      ...
      console.error(`[EntityManager] Error getting entity metadata:`, error);
      return null;
    }
  }
   ...
   */
  private isReferenceField(fieldType: string): boolean {
    return [
      'priority_option',
      'status_option', 
      'category_option',
  ...
  private async buildReferenceLookupMaps(orgId: string, referenceFields: Record<string, any>): Promise<Record<string, Record<string, any>>> {
    ...
    for (const [fieldName, fieldInfo] of Object.entries(referenceFields)) {
      try {
        ...
        console.warn(`[EntityManager] Failed to build lookup map for ${fieldName}:`, error);
      }
    }
  ...
  private async getSystemOptionLookup(optionType: string, archetype: string): Promise<Record<string, any>> {
    try {
      ...
      console.error(`[EntityManager] Error getting system option lookup for ${optionType}:`, error);
      return {};
    }
  }
  ...
  private async getCustomOptionLookup(orgId: string, fieldName: string): Promise<Record<string, any>> {
    try {
      ...
      console.error(`[EntityManager] Error getting custom option lookup for ${fieldName}:`, error);
      return {};
    }
  }
