...
export function syncProcessView(input: ViewActorInput): ViewActorOutput {
  ...
  console.log('🔍 syncProcessView: Processing', {
    entityCount: input.entities.length,
    columnCount: input.columns.length,
    hasResolvers: !!input.relationshipResolvers,
    resolverKeys: input.relationshipResolvers ? Object.keys(input.relationshipResolvers) : []
  });
  ...
  const relationshipColumns = input.relationshipResolvers ? 
    input.columns.filter(col => {
      ...
      const isRelationship = cellType?.startsWith('relationship');
      const hasResolver = !!input.relationshipResolvers![col.id];
      
      if (isRelationship) {
        console.log('🔍 syncProcessView: Relationship column check', {
          columnId: col.id,
          field: col.field,
          cellType,
          hasResolver,
          resolverExists: !!input.relationshipResolvers![col.id]
        });
      }
      
      return isRelationship && hasResolver;
    }).map(col => ({
  ...
  console.log('🔍 syncProcessView: Relationship columns found', {
    ...
    const resolvedData = relationshipColumns.length > 0 ? 
      relationshipColumns.reduce((data, { field, resolver, resolvedKey }) => {
        ...
        if (value != null) {
          ...
          if (index < 2) {
            console.log('🔍 syncProcessView: Resolving relationship', {
              entityId: entity.id,
              field,
              value,
              resolved,
