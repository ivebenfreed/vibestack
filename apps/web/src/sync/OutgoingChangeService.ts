...
export class OutgoingChangeService {
  ...
  constructor(
    ...
    console.log('[OutgoingChangeService] Using shared DataSource from PGliteProvider context');
  }
  ...
  async trackEntityChange(table: string, operation: 'insert' | 'update' | 'delete', entity: any): Promise<void> {
    ...
    console.log(`[OutgoingChangeService] Tracking entity change: ${table}:${operation}:${entityId}`);
    ...
    try {
      ...
      console.log(`[OutgoingChangeService] Stored change in database: ${localChangeId} for ${table}:${entityId}`);
      ...
      if (this.messageSender) {
        ...
        console.log(`[OutgoingChangeService] Change queued, will send when WebSocket is ready: ${localChangeId}`);
      }
      
    } catch (error) {
      console.error('[OutgoingChangeService] Error tracking change:', error);
      throw error;
    }
  }
  ...
  async detectAndQueueChanges(): Promise<number> {
    ...
    try {
      ...
      console.log(`[OutgoingChangeService] Found ${localChanges.length} unprocessed LocalChanges`);
      ...
      console.log(`[OutgoingChangeService] Optimized ${localChanges.length} changes to ${optimizedChanges.length}`);
        ...
        if (!this.pendingChanges.has(localChangeId)) {
          ...
          if (typeof data === 'string') {
            try {
              ...
              console.error(`[OutgoingChangeService] Failed to parse LocalChanges.data for change ${localChange.id}:`, e);
              data = {};
            }
          }
      ...
      if (queuedCount > 0) {
        ...
        if (this.messageSender) {
          ...
          console.log(`[OutgoingChangeService] ${queuedCount} changes queued, will send when WebSocket is ready`);
        }
      }
  ...
  handleChangesReceived(message: BaseServerMessage): void {
    ...
    console.log(`[OutgoingChangeService] 📥 Server acknowledged receipt of ${receivedMessage.changeIds?.length || 0} changes`);
    console.log(`[OutgoingChangeService] 📥 Received message:`, receivedMessage);
  }
  ...
  async handleChangesApplied(message: BaseServerMessage): Promise<void> {
    console.log(`[OutgoingChangeService] 🔄 handleChangesApplied called with message type: ${message.type}`);
    ...
    console.log(`[OutgoingChangeService] ✅ Server applied changes. Success: ${appliedMessage.success}. Applied: ${appliedEntityIds.length}. Error: ${appliedMessage.error || 'None'}`);
    console.log(`[OutgoingChangeService] ✅ Applied message:`, appliedMessage);
    ...
    if (appliedMessage.success) {
      ...
      for (const entityId of appliedEntityIds) {
        console.error(`[OutgoingChangeService] Server failed to apply change for entity ${entityId}: ${appliedMessage.error}`);
        const localChangeIds = await this.findLocalChangesByEntityId(entityId);
  ...
  handleServerError(message: ServerErrorResponseMessage | BaseServerMessage): void {
    ...
    console.error(`[OutgoingChangeService] Received server error: ${errorCode} - ${errorMessage}. Original Msg ID: ${originalMessageId}`);
  ...
  private async optimizeOutgoingChanges(localChanges: LocalChanges[]): Promise<LocalChanges[]> {
    ...
    for (const change of localChanges) {
      ...
      if (!entityId) {
        console.warn(`[OutgoingChangeService] Change ${change.id} missing entity_id in data during optimization.`);
        continue;
      }
    ...
    if (processedDueToOptimization.length > 0) {
      console.log(`[OutgoingChangeService] ${processedDueToOptimization.length} changes were optimized out or merged.`);
  ...
  private checkSentChanges(): void {
    ...
    this.sentChanges.forEach((info, localChangeId) => {
      if (now - info.timestamp > info.timeout) {
        if (info.attempt >= MAX_RETRY_ATTEMPTS) {
          console.error(`[OutgoingChangeService] Change ${localChangeId} permanently failed after ${info.attempt} attempts. Marking as failed.`);
          this.sentChanges.delete(localChangeId);
          permanentlyFailed.push(localChangeId);
        } else {
          console.warn(`[OutgoingChangeService] Change ${localChangeId} timed out (attempt ${info.attempt}). Will retry.`);
          this.sentChanges.delete(localChangeId);
    ...
    if (permanentlyFailed.length > 0) {
      this.markLocalChangesAsProcessed(permanentlyFailed, false, `timeout_after_${MAX_RETRY_ATTEMPTS}_attempts`)
        .catch((error: Error) => {
          console.error('[OutgoingChangeService] Error marking permanently failed changes:', error);
        });
    }
  ...
  private async queryUnsyncedChanges(): Promise<TableChange[]> {
    try {
      console.log('[OutgoingChangeService] Querying LocalChanges table for unsynced changes...');
      ...
      console.log(`[OutgoingChangeService] Found ${unprocessed.length} unsynced changes in LocalChanges table`);
  ...
  private async markChangesAsSynced(changeIds: string[]): Promise<void> {
    ...
    try {
      console.log(`[OutgoingChangeService] Marking ${changeIds.length} LocalChanges as processed`);
      ...
      console.log(`[OutgoingChangeService] Successfully marked ${changeIds.length} changes as processed`);
  ...
  private async findLocalChangesByEntityId(entityId: string): Promise<string[]> {
    try {
      ...
      console.error(`[OutgoingChangeService] Error finding LocalChanges for entity ID ${entityId}:`, error);
      return [];
    }
  }
  ...
  private async markLocalChangesAsProcessed(changeIds: string[], success: boolean, reason: string): Promise<void> {
    ...
    try {
      console.log(`[OutgoingChangeService] Marking ${changeIds.length} LocalChanges as processed due to: ${reason}`);
      ...
      console.log(`[OutgoingChangeService] Successfully marked ${changeIds.length} changes as processedSync=${statusToSet}`);
      ...
      console.error(`[OutgoingChangeService] Error marking changes as processed (reason: ${reason}):`, error);
      throw error;
    }
  }
  ...
  private async sendChangeBatch(batch: PendingChange[]): Promise<void> {
    ...
    try {
      ...
      for (const pending of batch) {
        ...
        console.log(`[OutgoingChangeService] Tracking change ${pending.id} (attempt ${attempt}, timeout ${timeout}ms)`);
      }
