...
import { 
  ...
  generateUniverseSection,
  generateWorldsSection,
  globalSidebarData,
...
export const sidebarNavigationStore = createStore({
  context: {
    ...
    lastWorldsHash: '', // Track changes to worlds for selective updates
  },
  ...
  on: {
    ...
    updateNavigationWithWorlds: (context, event: { 
      projects: Project[], 
      ...
      if (import.meta.env.DEV) {
        log.info(`[SidebarNavigationStore] Recomputing navigation with worlds...`, {
          timestamp: new Date().toISOString(),
          projectsCount: event.projects.length,
          worldsCount: Object.keys(event.worldData || {}).length,
          universeCount: Object.keys(event.universeData || {}).length
        })
      }
      
      const sectionsMap: Record<string, NavGroup[]> = {}
      
      // Add static sections from globalSidebarData (home, settings, debug)
      globalSidebarData.forEach(section => {
        if (!['universe', 'worlds', 'projects', 'entities'].includes(section.id)) {
          sectionsMap[section.id] = section.navGroups
        }
      })

      // Add universe section with real data
      const universeSection = generateUniverseSection(event.universeData, event.worldData)
      sectionsMap['universe'] = universeSection.navGroups

      // Add worlds section with real data
      const worldsSection = generateWorldsSection(event.worldData)
      sectionsMap['worlds'] = worldsSection.navGroups
      ...
      if (import.meta.env.DEV) {
        log.info(`[SidebarNavigationStore] ✅ Navigation with worlds computed in ${computationTime.toFixed(2)}ms`)
      }
      
      const projectsHash = event.projects.map(p => `${p.id}-${p.updatedAt}`).join(',')
      const worldsHash = Object.values(event.worldData || {}).map((w: any) => `${w.id}-${w.updated_at}`).join(',')
...
export const useSidebarNavigationStore = {
  ...
  updateNavigation: (projects: Project[]) => 
    sidebarNavigationStore.trigger.updateNavigation({ projects }),
  updateNavigationWithWorlds: (projects: Project[], schema: EntitySchema | null, universeData: any, worldData: any) =>
    sidebarNavigationStore.trigger.updateNavigationWithWorlds({ projects, schema, universeData, worldData }),
