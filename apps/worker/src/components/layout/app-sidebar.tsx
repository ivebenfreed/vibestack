...
const AppSidebarInternal = React.memo(function AppSidebarInternal({ collapsible = 'icon', variant = 'floating', ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const activeSection = React.useMemo(() => {
    if (location.pathname.startsWith('/entities')) {
      return 'entities'
    } else if (location.pathname.startsWith('/universe')) {
      return 'universe'
    } else if (location.pathname.startsWith('/worlds')) {
      return 'worlds'
  ...
  const shouldShowExpensiveContent = ['projects', 'settings', 'home', 'universe', 'worlds', 'entities'].includes(activeSection)
