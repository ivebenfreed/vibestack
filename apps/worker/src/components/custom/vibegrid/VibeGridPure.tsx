...
export function VibeGridPure<T extends Record<string, any> = any>(
  ...
  useEffect(() => {
    ...
    try {
      log.info('🚀 Initializing VibeGridPure with pure observables', {
        tableId,
        entityType,
      ...
      const checkPersistLoaded = () => {
        ...
        log.info('🔄 Checking persistence loaded state', { 
          entityType, 
          isPersistLoaded: isLoaded 
        });
        
        if (isLoaded) {
          log.info('✅ Persistence loaded, initializing renderer', { entityType });
          ...
          log.info('✅ SimplePassiveRenderer with enhanced selection initialized successfully');
      ...
      log.error('❌ Failed to initialize VibeGridPure', err);
      setError(errorMsg);
    }
    ...
    return () => {
      if (rendererRef.current) {
        log.info('🧹 Cleaning up VibeGridPure');
