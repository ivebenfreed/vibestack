...
const fileLog = log('archive/deprecated-components/timeline/utils/timelineGrid.ts');
...
export class TimelineGrid {
  ...
  configureLanes(nodes: Node[]): GridLane[] {
    ...
    fileLog.info(`🔄 CONFIGURING LANES for ${taskNodes.length} tasks (reorder by start date)`)
    ...
    sortedTasks.forEach((node, taskIndex) => {
      ...
      fileLog.info(`🏁 LANE ORDERED: "${taskLane.name}" at position ${taskIndex + 1} Y=${taskLane.y} (start: ${taskData.startDate})`)
    })
  ...
  getLaneForNode(nodeId: string): GridLane | null {
    ...
    if (foundLane) {
      fileLog.info(`🔍 LANE LOOKUP SUCCESS: Found lane "${foundLane.name}" for node ${nodeId}`)
    } else {
      fileLog.info(`🚫 LANE LOOKUP FAILED: No lane found for node ${nodeId}`)
      fileLog.info(`Available lanes:`, this.lanes.map(l => ({ id: l.id, name: l.name, nodeIds: l.nodes.map(n => n.id) })))
    }
  ...
  updateBounds(startDate: Date, endDate: Date): void {
    fileLog.info(`📐 GRID: Updating bounds from ${this.config.startDate.toISOString().split('T')[0]} to ${startDate.toISOString().split('T')[0]} - ${endDate.toISOString().split('T')[0]}`)
