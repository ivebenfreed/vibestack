...
export class RelationshipFieldHandler {
  ...
  static convertToRelationshipMetadata(
    ...
    const relationshipType = this.inferRelationshipType(fieldName, fieldType);
    const targetEntityType = this.inferTargetEntityType(fieldName, fieldType);
  ...
  static async createRelationship(
    ...
    await kysely
      ...
      .onConflict((oc) => oc
        .columns(['source_entity_type', 'source_entity_id', 'relationship_type', 'target_entity_type', 'target_entity_id'])
  ...
  private static async createRelationshipTable(kysely: Kysely<any>, orgId: string): Promise<void> {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS ${sql.table(tableName)} (
        ...
        CONSTRAINT unique_active_relationship UNIQUE (
    ...
    // Create indexes
    await sql`CREATE INDEX IF NOT EXISTS idx_${sql.raw(tableName)}_source 
      ON ${sql.table(tableName)}(source_entity_type, source_entity_id) 
      WHERE valid_until IS NULL`.execute(kysely);
      
    await sql`CREATE INDEX IF NOT EXISTS idx_${sql.raw(tableName)}_target 
      ON ${sql.table(tableName)}(target_entity_type, target_entity_id) 
      WHERE valid_until IS NULL`.execute(kysely);
      
    await sql`CREATE INDEX IF NOT EXISTS idx_${sql.raw(tableName)}_type 
      ON ${sql.table(tableName)}(relationship_type) 
      WHERE valid_until IS NULL`.execute(kysely);
  }
